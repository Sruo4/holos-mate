    ...
    <div class="data-container">
      ...
      <div v-if="data.length > 0">
        <div v-for="(record, index) in data" :key="index" class="data-record">
          ...
          <div v-if="type === 'psychological'" class="record-score">{{ recordValueLabel }}: {{ (record as PsychologicalRecord).value }}</div>
          ...
          <div v-else-if="type === 'symptoms'" class="record-score">{{ recordValueLabel }}: {{ (record as SymptomsRecord).value }}</div>
        </div>
      </div>
      <div v-else>
        <p>没有数据。</p>
